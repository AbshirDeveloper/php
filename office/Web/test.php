...
<?php
	...
	$result1 = $public->select_all("customer");
    $result = $public->select_all("products");
    ...
    $phpexcel->getactivesheet()->setcellvalue("A{$cel}", "Phone");
    $phpexcel->getactivesheet()->setcellvalue("A{$cel}", "ID");
    $phpexcel->getactivesheet()->setcellvalue("A{$cel}", "Total Weight");

    $weight = $public->select_sum_column('products', 'weight', $array['id']);

    $phpexcel->getactivesheet()->setcellvalue("B{$cel}", $abshir['first_name'] . " " . $abshir['last_name']);
    $phpexcel->getactivesheet()->setcellvalue("B{$cel}", $abshir['phone']);
    $phpexcel->getactivesheet()->setcellvalue("B{$cel}", $abshir['id']);
    $phpexcel->getactivesheet()->setcellvalue("B{$cel}", $weight);


	$phpexcel->getactivesheet()->setcellvalue("B2", "AWB");
	$phpexcel->getactivesheet()->setcellvalue("C2", "Length");
	$phpexcel->getactivesheet()->setcellvalue("D2", "Width");
	$phpexcel->getactivesheet()->setcellvalue("E2", "Height");
    $phpexcel->getactivesheet()->setcellvalue("F2", "Quantity");
    $phpexcel->getactivesheet()->setcellvalue("G2", "Weight");
	...
	$phpexcel->getactivesheet()->setcellvalue("B{$cell}", $answer['name']);
	$phpexcel->getactivesheet()->setcellvalue("C{$cell}", $answer['barcode']);
	$phpexcel->getactivesheet()->setcellvalue("D{$cell}", $answer['amount']);
	$phpexcel->getactivesheet()->setcellvalue("E{$cell}", $answer['total']);
...
<br/>
<form>
	Name <input type="text" name="nane">
