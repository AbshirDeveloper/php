...
$try = "select * from cart_angular where id=$data->id";

$find = mysqli_query($connection, $try);
while($ani = mysqli_fetch_array($find)){
    
$new_total = $data->quantity * $ani['cost'];
$sql = "UPDATE cart_angular
SET quantity='$data->quantity', total=$new_total WHERE id=$data->id limit 1";    
