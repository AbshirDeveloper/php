...
$debit = 0;
$balance = $data->amount - $debit;
$maanta = time();
$sql = "INSERT INTO amano (date, description, debit, deposit, balance, idbalance, customer_id)
VALUES ('$maanta','$data->des', '$debit', '$data->amount', $balance, $idbalance, $data->id)";
if($data->des != null){
...
if($idbalance >= 0 ){
$message = "Waxaad dhigatay $".$data->amount." maanta oo taariikhdu tahay {$maanta}. Haraaga akoonkaagu waa $".$idbalance.". Lasoco xisaabtaada";
} else {
$real_bal = $idbalance * (-1);
$message = "Waxaad bixisay $".$data->amount." maanta oo taariikhdu tahay {$maanta}. Waxaa wali kugu harsan $".$real_bal.". Lasoco xisaabtaada";   
}
...
$pertain = "Amano transaction with {$name_amano} (Deposit)";
