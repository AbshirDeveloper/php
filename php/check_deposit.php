...
$sql_cash = "select sum(amount) as total from checks where status = 'Deposited'";
    $result_cash = mysqli_query($connection, $sql_cash);
while ($row_cash = mysqli_fetch_assoc($result_cash)){
    $sum_cashe = $row_cash['total'];
}

$sql_cash_count = "SELECT count(*) as total from checks where status = 'Deposited'";
...
VALUES ('$today_real', 'Checks', $sum_cashe_count, $sum_cashe, '$initials', $unique)";
$qry = mysqli_query($connection, $sql);

$w = "update checks set status = 'staged', uniq = {$unique} where status = 'Deposited'";
